/****** Mersenne Twister code adpoted from code by Agner Fog
Date Included: 2013-3-13
Included by: Geoff Lee

Included to overcome system dependent random functions, & replace the old 
random function (not very reliable).
...
int Random::IRandom(int min, int max) {
   ...
   int r = int((double)(uint32_t)(max - min + 1) * randReal() + min); 
   if (r > max) r = max;
   return r;
}


int Random::IRandomX(int min, int max) {
   // Output random integer in the interval min <= x <= max
